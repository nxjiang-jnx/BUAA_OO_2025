...
public class Parser {
    ...
    public OrdinaryFun parseOrdinaryFunFactor() {
        OrdinaryFun ordinaryFun = new OrdinaryFun();
        ordinaryFun.setName(lexer.peek());
        lexer.next();   // 吃掉'('
        lexer.next();
        ordinaryFun.setFactor1(parseFactor());
        if (lexer.peek().equals(",")) {
            lexer.next();
            ordinaryFun.setFactor2(parseFactor());
        }
        lexer.next();   // 吃掉')'
        return ordinaryFun;
    }

    public Derivation parseDerivationFactor() {
        lexer.next();   // 吃掉'd'
        lexer.next();   // 吃掉'x'
        lexer.next();   // 吃掉'('
        Derivation derivation = new Derivation();
        derivation.setExpr(parseExpr());
        lexer.next();   // 吃掉')'
