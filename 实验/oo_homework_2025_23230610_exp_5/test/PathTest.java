...
public class PathTest {//包含题目[3]
    ...
    public void testExtractLoopPath() {
        ...
        for (int i = 0; i < oldNodes.size(); i++) {
            for (int j = i + 1; j < oldNodes.size(); j++) {
                if (oldNodes.get(i).equals(oldNodes.get(j))) {
                    fl = true;
                    break;
                }
            }
        ...
        if (fl && path.isValid()) {
            ...
            public normal_behavior
             */
            Path loopPath = path.extractLoopPath();
             ...
             */
            assertTrue(loopPath instanceof Path);

            boolean exist = false;
            for (int i = 0; i < oldNodes.size() - 1; i++) {
                exist = false;
                for (int j = 0; j < loopPath.getNodes().size(); j++) {
                    if (i + j < oldNodes.size() && oldNodes.get(i + j).equals(loopPath.getNodes().get(j))) {
                        exist = true;
                    } else {
                        exist = false;
                        break;
            ...
            if (loopPath.getNode(0) != loopPath.getNode(loopPath.size() - 1)) {
                exist = false;
            }
        ...
        if (forall || !path.isValid()) {
            ...
            ensures \result == null;
             */
            assertNull(path.extractLoopPath());
        }
